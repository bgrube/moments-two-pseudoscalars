...
\pm$, the \emph{reflectivities}, are chosen such that they correspond
to the naturality of the exchange particle in the scattering
process.\todo{sign chosen for pion beam}  The normalization factor
\begin{equation}
  \label{eq:refl_norm}
  \mathcal{N}_m
  = \begin{cases*}
...
Sec.~5.2 of \refCite{Varshalovich:1988krb}.}
\begin{equation}
...
\todoInl{Discuss this after having introced normalization}
...
m}$.  \Cref{eq:diffraction_moments_pw} also shows that~$L$ and~$M$ are
...
\begin{equation}
  ...
  = \prescript{\refl}{}{\varrho}^{\ell'\, \ell}_{m'\, m}.
\end{equation}
Using this together with \cref{eq:diffraction_moments_pw_norm}
and\footnote{See Eq.~(10) in Sec.~8.4.3 of
\refCite{Varshalovich:1988krb}.}
\begin{equation}
...
\cref{eq:diffraction_moments_pw_norm}.\footnote{%
  ...
  \begin{equation}
    ...
    = (-1)^M\, H(L, -M).
  \end{equation}\todoInl{move this into main text}
...
Using \cref{eq:diffraction_moments_pw_norm} and the symmetry
...
\Cref{eq:diffraction_moments_norm}, we obtain
\begin{align}
  \sqrt{\frac{4 \pi}{2 L + 1}}\, \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, Y_L^{M *}(\Omega)
  \mustBeEq{}&
  \sqrt{\frac{4 \pi}{2 L + 1}}\, \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, Y_L^M(\Omega) \\
