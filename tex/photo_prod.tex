...
\end{equation}
This is equivalent to \cref{eq:diffraction_integral_matrix_mc_weighted}.  Note
...
\todo{rewrite $\rBrk[0]{\hat{\vect{H}}_\text{meas}}_{i, L M}$ in
\cref{eq:photoprod_moment_meas_unpol,eq:photoprod_moments_meas_pol} in
terms of basis functions}
The measured moments are estimated from the
data using the same approach as in
\cref{sec:diffraction:estimation_uncert}, \ie by replacing the
integrals in
...
$N_\text{meas}$~is fixed and hence not a random
variable.\footnote{Note that in the case of weighed events, the
covariances in \cref{eq:photoprod_H0_cov_unweighted} are non-zero,
because in this case $\rBrk[0]{\hat{\vect{H}}_\text{meas}}_{0, 0 0} =
N_\text{sig}$ (see \cref{eq:photoprod_norm_00}) and for
fixed~$N_\text{meas}$ the number of signal events is a random
variable.}
