...
\refCite{Mathieu:2019fts}.  Note that in contrast to
...
from Eq.~(A10) to Eq.~(A11) in \refCite{Mathieu:2019fts}.  Note that
...
conditions.  However, in practice one often wants to combine several
data samples that were taken with different experimental conditions.
This means that in general the photon-beam polarization~$P_\gamma$,
the detection efficiency $\eta(\Omega, \Phi)$, and consequently also
the acceptance integral matrix~$\mat{I}^\text{acc}$ are functions of
time.

Assume we have $N_\text{s}$~data samples, indexed
...
events.}  For each data sample, which was obtained using constant
...
\end{equation}
We use \cref{eq:complex_uncert_prop} to calculate the covariance
matrix~$\prescript{(j)}{}{\vphantom{\vect{H}}}\hat{\underaccent{\bar}{\covMatSym}}_{\!\hat{\vect{H}}}$
of~$\prescript{(j)}{}{\vphantom{\vect{H}}}\hat{\vect{H}}$.

The total physical intensity distribution is given by the sum of the
...
\begin{equation}
  ...
  \hat{H}_i^\text{tot}
  = \sum_{j = 1}^{N_\text{s}} \prescript{(j)}{}{\vphantom{H}}\hat{H}_i
...
\Set[1]{\prescript{(j)}{}{\vphantom{\vect{H}}}\hat{H}_0(0, 0)} match
or by comparing the normalized moments.

To propagate the uncertainties from
\Set[1]{\prescript{(j)}{}{\vphantom{\vect{H}}}\hat{\vect{H}}}
...
\end{equation}with the augmented covariance matrix
\begin{equation}
...
\set{\prescript{(j)}{}{\vphantom{\vect{H}}}\hat{\vect{H}}} of physical
moments
\begin{equation}
...
\end{equation}
Here, $n$~is the number of moments given by~$L_\text{max}$.  Since the
data samples are statistically independent, the Hermitian covariance
matrix~$\hat{\covMatSym}_{\!\hat{\vect{H}}_\text{s}}$ and the
pseudo-covariance
matrix~$\tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}_\text{s}}$ that
...
\begin{equation}
  ...
  \quad\text{and}\quad
  \tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}_\text{s}}
  = \diag\rBrk[1]{\prescript{(1)}{}{\vphantom{\vect{H}}}\tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}}
  \ldots,
  \prescript{(N_\text{s})}{}{\vphantom{\vect{H}}}\tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}}}.
\end{equation}
Here,
$\prescript{(j)}{}{\vphantom{\vect{H}}}\hat{\covMatSym}_{\!\hat{\vect{H}}}$~and~$\prescript{(j)}{}{\vphantom{\vect{H}}}\tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}}$
...
augmented Jacobian matrix is block-diagonal (see
...
\end{equation}
Because of the simple sum over the data samples in
\cref{eq:photoprod_data_sample_phys_moments_total}, the Jacobian
matrix~$\mat{J}$ reduces to the identity matrix, \ie
\begin{equation}
...
\begin{equation}
  ...
  \in \mathbb{C}^n.
\end{equation}
From \cref{eq:photoprod_data_sample_phys_moments_total} we get
\begin{equation}
...
\begin{equation}
  ...
  = \begin{pmatrix}
    ...
    \sum_{j = 1}^{N_\text{s}}
    \prescript{(j)}{}{\vphantom{\vect{H}}}\tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}} \\[1ex]
    \sum_{j = 1}^{N_\text{s}}
    \prescript{(j)}{}{\vphantom{\vect{H}}}\tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}}^* &
    \sum_{j = 1}^{N_\text{s}}
    \prescript{(j)}{}{\vphantom{\vect{H}}}\hat{\covMatSym}_{\!\hat{\vect{H}}}^*
  \end{pmatrix}
  = \begin{pmatrix}
    \hat{\covMatSym}_{\!\hat{\vect{H}}_\text{tot}} &
    \tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}_\text{tot}} \\[1ex]
    \tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}_\text{tot}}^* &
    \hat{\covMatSym}_{\!\hat{\vect{H}}_\text{tot}}^*
  \end{pmatrix}.
\end{equation}
Here,
$\hat{\covMatSym}_{\!\hat{\vect{H}}_\text{tot}}$~and~$\tilde{\hat{\covMatSym}}_{\!\hat{\vect{H}}_\text{tot}}$
are the Hermitian and the pseudo-covariance matrix
of~$\hat{\vect{H}}_\text{tot}$, respectively. This result is not
surprising.  It is the generalization of the fact that the variance of
a sum of random variables is given by the sum of the variances of
these variables.
