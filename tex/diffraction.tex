...
obtain the \emph{normalized moments}, which are defined by $H_norm(0,
0) = 1$.} and with the chosen normalization (see
...
and only a finite set of physical moments with $(L' M')$ quantum
numbers contribute to each of the measured moments.  We can hence
rewrite \cref{eq:diffraction_moments_meas2} as a matrix equation in
...
\end{equation}
where $\mat{I}^\text{acc}$ is the acceptance integral matrix.  From
the above it is clear that the measured moments are linear
combinations of the physical moments, \ie the acceptance mixes the
physical moments into the measured moments.
...
to inflated uncertainty estimates due to small eigenvalues
of~$\mat{I}^\text{acc}$.  In case the detection efficiency is close to
zero over wide ranges of the phase space, $\mat{I}^\text{acc}$~may
become (nearly) singular.  In this case, certain moments may become
