...
and $\mathcal{I}(\Omega)$ is the physical intensity distribution.  To
...
M}$\footnote{By convention, the real-valued spherical harmonics are
written with $M$~as a subscript.} as defined, for example, in
\refsCite{wikipedia:sphericalHarm,Jones:2023} with $-L \leq M \leq
+L$.}\footnote{\Confer \cref{eq:diffraction_moments_norm}; equivalent
to Eq.~(B2) in \refCite{E852:1999xev}.}
\begin{equation}
...
limiting factor, we will not follow this route.}  This is complicated
by the fact that in general the measured moments
...
show that this is true, although the scatter of the points is larger
than that of the real parts.
