...
according to $\mathcal{I}(\Omega)$.\footnote{%
  ...
  \end{equation}
  where in the last step we used the fact that the samples are
...
\cref{eq:diffraction_norm_00} we get
\begin{equation}
...
acceptance matrix $\rBrk[0]{\mat{I}^\text{acc}}^{-1}$ in
...
\begin{align}
  \rBrk[1]{\hat{\covMatSym}_{\!\hat{\vect{H}}_\text{meas}}}_{L M, L' M'}
...
$\mathcal{I}(\Omega)$ from a set of partial waves that consists of the
...
of \num{1000}~events (see \cref{fig:diffraction_study_data}).  This
sample is used to calculate the values of the measured
moments~$\hat{\vect{H}}_\text{meas}$ using
\cref{eq:diffraction_moments_meas_estimate} as well as their augmented
...
using \cref{eq:diffraction_integral_matrix} and \num{e7}~events
sampled randomly from the detection efficiency $\eta(\Omega)$ in
\cref{eq:diffraction_study_acc1}.  Using this matrix, the physical
...
extracted from the Monte Carlo data and compares them to the expected
values calculated from the partial-wave amplitudes in
...
are expected to be real-valued (see
\cref{eq:diffraction_moment_real}), we also verify that their
imaginary parts are consistent with zero, which is indeed the case
(see
...
\begin{figure}[tbp]
  ...
  \subfloat[][]{%
    ...
    \includegraphics[width=\twoPlotWidth]{acc_1/hCompareRe}%
  }%
  \hfill%
  \subfloat[][]{%
    \label{fig:diffraction_study_comparison_im}%
    \includegraphics[width=\twoPlotWidth]{acc_1/hCompareIm}%
  }%
  \\
  \subfloat[][]{%
    \label{fig:diffraction_study_residual_re}%
    \includegraphics[width=\twoPlotWidth]{acc_1/hResidualsRe}%
  }%
  \hfill%
  \subfloat[][]{%
    \label{fig:diffraction_study_residual_im}%
    \includegraphics[width=\twoPlotWidth]{acc_1/hResidualsIm}%
  }%
  ...
  lines).  \subfloatLabel{fig:diffraction_study_comparison_re}~shows
  the real part,
  \subfloatLabel{fig:diffraction_study_comparison_im}~the imaginary
  part.  Note that moments with $M > 2$ and/or $L > 4$ are expected to
  be zero due to the chosen wave set.  Lower row: corresponding
  residuals.\todoInl{Fix/discuss $H(0, 0)$.}}
  \label{fig:diffraction_study_output}
\end{figure}

\todoInl{Discuss acceptance correction using moment decomposition of $\eta^{-1}(\Omega)$}
\todoInl{Add reference to code repo}
