...
\cref{eq:photoprod_moments}, we get
\begin{align}
...
\subsubsection{Estimating moments using extended maximum-likelihood fits}%
...
To speed up the calculation of the likelihood function, we can
precalculate~$\vect{I}_\text{acc}$ and the basis-function vectors
\set{\vect{f}(\Omega_k, \Phi_k)} that enter
\cref{eq:photoprod_likelihood_2}, so that
\begin{equation}
...
moments corresponds to the set of moment values that maximize the
likelihood function, \ie
\begin{equation}
...
operator (see \cref{eq:photoprod_moments})
\begin{equation}
