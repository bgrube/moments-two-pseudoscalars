...
$Y_\ell^m(\Omega)$~\cite{wikipedia:sphericalHarm}.
...
\cref{eq:diffraction_moments}), they inherit the symmetry properties
...
expressing the moments in terms of spin-density elements using the
relation\footnote{See Eq.~(A.15) in \refCite{Chung:1971ri}.}
\begin{equation}
...
choose a different formalization for $H(L, M)$.}
\begin{equation}
...
\end{equation}
Using this plus the fact that the spin-density matrix is Hermitian,
\ie
\begin{equation}
...
\begin{align}
  ...
  ={}& \sqrt{\frac{2 L + 1}{4 \pi}} \sum_{\substack{\ell m \\ \ell' m'}}^\infty
    ...
    \varrho^{\ell'\, \ell}_{m'\, m} \notag
  \\
  ={}& \dUnderbrace{(-1)^{2 L - M}}{= (-1)^M}\, \sqrt{\frac{2 L + 1}{4 \pi}}
...
to \cref{eq:diffraction_moments_pw}.

The symmetry relation in \cref{eq:diffraction_moment_sym_1} ensures
that the intensity function in \cref{eq:diffraction_intensity_moments}
is real-valued:
\begin{align}
  \mathcal{I}(\Omega)
  ={}& \sum_{L = 0}^\infty \sum_{M = -L}^{+L} H(L, M)\, Y_L^M(\Omega) \notag
  \\
  ={}& \sum_{L = 0}^\infty \sBrk[4]{\rdUnderbrace{\sum_{M = -L}^{-1} H(L, M)\, Y_L^M(\Omega)}%
  ...
  \\
  ={}& \sum_{L = 0}^\infty \sBrk[4]{H(L, 0)\, Y_L^0(\Omega) + \sum_{M = +1}^{+L}
  ...
  ={}& \sum_{L = 0}^\infty \sum_{M = 0}^{L} (2 - \delta_{M 0})\, \Re{H(L, M)\, Y_L^M(\Omega)}.
\end{align}
...
and compared to \cref{eq:diffraction_moments_pw}.
...
\cref{eq:diffraction_moments} on both sides, we obtain
\begin{align}
  \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, Y_L^{M *}(\Omega)
  \mustBeEq{}&
  \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, Y_L^M(\Omega) \\
  \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, y_L^M(\theta) \sBrk{\cos(M \phi) + \imag \sin(M \phi)}
  \mustBeEq{}&
  \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, y_L^M(\theta) \sBrk{\cos(M \phi) - \imag \sin(M \phi)}.
\end{align}
Consequently,
\begin{align}
  ...
  0
  ={}& \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, y_L^M(\theta)\, \sin(M\, \phi) \\
  \intertext{and}
  \label{eq:diffraction_moments_real}
  H(L, M)
  ={}& \int_{4 \pi}\!\!\! \dif{\Omega}\, \mathcal{I}(\Omega)\, y_L^M(\theta)\, \cos(M\, \phi).
\end{align}
...
It is often advantageous to express \cref{eq:diffraction_intensity} in
...
spherical harmonics,\footnote{See Eq.~(1) in Sec.~5.2 of
\refCite{Varshalovich:1988krb}.}
\begin{equation}
...
For convenience, we introduce the real-valued function
\begin{equation}
...
\cref{eq:diffraction_moments_pw_refl}.  However, in general the converse is
...
partial-wave amplitudes given by \cref{eq:diffraction_moments_pw_refl,},
...
m}$.  \Cref{eq:diffraction_moments_pw_refl} also shows that~$L$ and~$M$ are
...
We can derive \cref{eq:diffraction_moments_pw_refl} also by inserting
...
\begin{multline}
  ...
  Y_L^{M *}(\Omega),
\end{multline}
which is equivalent to \cref{eq:diffraction_moments_pw_refl}.
...
\cref{eq:diffraction_moments_pw_refl} and
\begin{equation}
...
defined in
\cref{eq:diffraction_intensity_moments_norm,eq:diffraction_moments_norm,eq:diffraction_moments_pw_refl_norm}.
