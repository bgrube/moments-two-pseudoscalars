...
We obtain the corresponding moments from the intensity distribution by
...
\label{sec:photoprod:moments_sym}
...
\cref{eq:spherical_harm_sym} and the definition of the moment operator in
\cref{eq:photoprod_moment_operator}, we get
\begin{align}
...
\refCite{Mathieu:2019fts}.  We use here the short-hand notation $\pm$
...
\subsubsection{Extended maximum-likelihood method}%
...
By collecting all non-zero physical moments and all basis functions
...
To estimate the moments from the measured intensity distribution, we
...
\end{align}
Note that in the case of perfect acceptance, \ie $\eta(\Omega, \Phi) =
...
The maximum-likelihood estimate~$\hat{\vect{H}}$ of the physical
...
basis-function vectors \set{\vect{f}(\Omega_k, \Phi_k)}.
...
To incorporate the event weights \set{w_k} for background subtraction,
...
\begin{equation}
  ...
  ^{\sum_{k = 1}^{N_\text{meas}} w_k}}}.
\end{equation}
Taking the logarithm and neglecting all terms independent of~\vect{H},
we obtain:
\begin{equation}
...
with random start values for~\vect{H}.

Particularly problematic for \textsc{Minuit} is the presence of
...
\begin{equation}
  ...
  \in \mathbb{C}^n.
\end{equation}
Because \cref{eq:photoprod_data_sample_phys_moments_total} is just a
