...
\begin{figure}[tbp]
  ...
  \subfloatLabel{fig:photoprod_study_weighted_discr_var_10M}~Generated
  distribution of the discriminatory variable.
  \subfloatLabel{fig:photoprod_study_weighted_data_10M}~Monte Carlo
  data sample of \num{2e7}~events randomly drawn from the distributions
  in~\subfloatLabel{fig:photoprod_study_weighted_intensity_sig}
  and~\subfloatLabel{fig:photoprod_study_weighted_intensity_bkg}
  according to
  \subfloatLabel{fig:photoprod_study_weighted_discr_var_10M}.}%
  %\label{fig:photoprod_study_weighted}%
\end{figure}

The angular distributions are disentagled by applying a
one-dimensional sideband subtraction in a hypothetical discriminating
variable, which covers the range $[-1, +1]$.\footnote{In practice this
could be, for example, an invariant mass distribution of one of the
daughter particles.}  In this variable, the signal is generated
according to a Gaussian, which is centered at~0 and has a standard
deviation of~0.1, while the background is generated uniformly.  The
signal band in the discriminating variable is $[-0.3, +0.3]$.  Events
in this range, are assigned a weight of $w_i = +1$.  The sidebands are
$[-1.0, -0.4]$ and $[+0.4, +1.0]$.  Events in these ranges are
assigned a weight of $w_i = -1/2$.\todo{add bands as well as signal
and background components to
...
background events.  \Cref{fig:photoprod_study_weighted_discr_var_10M}
shows the resulting distribution of the discriminatory variable and
\cref{fig:photoprod_study_weighted_data_10M} the angular distribution.
As expected, we recover the angular distribution of the signal
component by applying the sideband subtraction (\confer
...
\cref{fn:cov_aug_numpy}.\todo{add bands as well as signal and
background components to
\cref{fig:photoprod_study_weighted_discr_var}}
...
\begin{figure}[tbp]
  ...
  the partial-wave amplitudes listed in
  \cref{tab:photoprod_study_waveset_bkg}.
  \subfloatLabel{fig:photoprod_study_weighted_discr_var}~Generated
  distribution of the discriminatory variable.
  \subfloatLabel{fig:photoprod_study_weighted_data_10M}~Monte Carlo
  data sample of \num{2e7}~events randomly drawn from the distributions
  in~\subfloatLabel{fig:photoprod_study_weighted_intensity_sig}
  and~\subfloatLabel{fig:photoprod_study_weighted_intensity_bkg}
  according to
  \subfloatLabel{fig:photoprod_study_weighted_discr_var}.}%
  %\label{fig:photoprod_study_weighted}%
\end{figure}
...
\todo{Discuss background contributions}
% \subsection{Background contributions}%
% \label{sec:photoprod:background}

% In practice, the data sample will not only contain the signal channel
% but also background contributions from other channels.  This means in
% general the measured intensity distribution is the sum of
% $N_\text{channel}$~components indexed by~$j$, \ie
% \begin{equation}
%   \label{eq:photoprod_intensity_background}
%   \intensity_\text{meas}(\Omega, \Phi)
%   = \sum_{j = 1}^{N_\text{channel}} \prescript{(j)}{}{\eta}(\Omega, \Phi)\, \prescript{(j)}{}{\intensity}(\Omega, \Phi),
% \end{equation}
% where, \wolog, $j = 1$ corresponds to the signal channel.  For $j >
% 1$, $\prescript{(j)}{}{\eta}(\Omega, \Phi)$ is the efficiency function
% for background channel~$j$ reconstructed as the signal channel and
% $\prescript{(j)}{}{\intensity}(\Omega, \Phi)$ is the physical
% intensity distribution of this background channel.
